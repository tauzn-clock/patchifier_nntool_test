...
{
	switch (Loc) {
		case AT_MEM_L3_DEFAULTRAM:
			AT_DEFAULTRAM_CL_COPY((AT_DEFAULTRAM_T *) L3_Device, (AT_DEFAULTRAM_EXT_ADDR_TYPE) (Addr+Offset), (AT_DEFAULTRAM_INT_ADDR_TYPE) L2_BufferAddr, Size, 0, L3_Event);
			AT_DEFAULTRAM_CL_WAIT((AT_DEFAULTRAM_T *) L3_Device, L3_Event);
			break;
#if (CONFIG_HYPERRAM == 1) || (CONFIG_SPIRAM == 1) || (CONFIG_APS256XXN == 1)
		case AT_MEM_L3_HRAM:
		...
		case AT_MEM_L3_OSPIRAM:
			...
			break;
#endif
#if (CONFIG_DRIVER_HYPERFLASH == 1) || (CONFIG_DRIVER_ATXP032 == 1) || (CONFIG_DRIVER_MX25U51245G == 1) || (CONFIG_DRIVER_24XX1025 == 1) || (CONFIG_DRIVER_MRAM == 1)
		...
		case AT_MEM_L3_OSPIFLASH:
			...
			break;
		case AT_MEM_L3_DEFAULTFLASH:
			AT_DEFAULTFLASH_FS_CL_COPY((AT_DEFAULTFLASH_FS_T *) L3_Device, (AT_DEFAULTFLASH_FS_EXT_ADDR_TYPE) (Addr+Offset), (AT_DEFAULTFLASH_FS_INT_ADDR_TYPE) L2_BufferAddr, Size, 0, L3_Event);
			AT_DEFAULTFLASH_FS_CL_WAIT((AT_DEFAULTFLASH_FS_T *) L3_Device, L3_Event);
			break;
        #if defined(__GAP_DRIVER_VFS__) && defined(CONFIG_DEVICE_TREE)
		case AT_MEM_L3_PRI1:
		case AT_MEM_L3_PRI2:
			AT_VFS_FS_CL_COPY((AT_VFS_FS_T *) L3_Device, (AT_VFS_FS_EXT_ADDR_TYPE) (Addr+Offset), (AT_VFS_FS_INT_ADDR_TYPE) L2_BufferAddr, Size, 0, L3_Event);
			AT_VFS_FS_CL_WAIT((AT_VFS_FS_T *) L3_Device, L3_Event);
			break;
        #endif
#endif
#if (CONFIG_DRIVER_MRAM == 1)
				...
				for (int d3=0; d3<D3; d3++) {
					...
					for (int d4=0; d4<D4; d4++) {
						...
						switch (ItemSize) {
							...
							case 2:
                                                                if (IsFloat) {
                                                                        if (ArgType == 3) // ARG_IEEE16
                                                                                FVal = *((f16 *) (Addr+Item));
                                                                        else if (ArgType == 4) // ARG_BFLOAT16
                                                                                FVal = *((f16a *) (Addr+Item));
                                                                        else // ARG_FLOAT
                                                                                FVal = *((F16 *) (Addr+Item));
                                                                } else {
					...
					for (int d4=0; d4<D4; d4++) {
						...
						while(!ReadVal) {
							...
							if (!TileItemsToRead||InBuffer==0) {
								if (!TileItemsToRead) { /* End of Tile */
								...
								if (Loc==AT_MEM_L2)
									Addr = BaseAddr + ReadSoFar;
								else
									Addr = AT_TensorGetNextPage(Loc, L3_Device, L3_Event, Size, L2_BufferAddr, BaseAddr, ReadSoFar);
								TileSizeToRead -= Size;
								ReadSoFar += Size;
								Byte = 0; InBuffer = Size; Mask = 0x01;
							}
							if (Mask==0x01) Val = *((unsigned char *) (Addr+Byte));
							if (ReadSparse) {
								if (Mask&Val) {
									ReadSparse = 0;
									CurVal = 1<<LUTBits;
								...
								if (Mask&Val?1:0) CurVal |= (1<<ReadingBit);
								ReadingBit++;
...
{
	char FileName[300];
	sprintf(FileName, "%s_ArgName%s_ItemSize%d_Dim%d_%dx%dx%dx%dx%d.dat", NodeName, ArgName, ItemSize, Dim, D0,D1,D2,D3,D4);
	int IsFloat = 0;
	if (ItemSize<0) {
		IsFloat = 1;
		ItemSize = -ItemSize;
	}
	int TotalSizeBytes = D0*D1*D2*D3*D4*ItemSize;
	int SizeToRead = TotalSizeBytes;
	int InBuffer=0;
	if (L2_BufferSize==0) L2_BufferSize = SizeToRead;
	int Item = 0;
	int ReadSoFar = 0;
	...
	printf("Node: %s, Argument: %s, Dim: %d, [%d][%d][%d][%d][%d] ItemSize: %d ----> Writing to: %s\n", NodeName, ArgName, Dim, D0,D1,D2,D3,D4, ItemSize, FileName);
	AT_SWITCH_FS_T fs;
	AT_FS_SEMIHOST_INIT(fs);

	void *File = AT_FS_SEMIHOST_OPEN_WRITE(fs, FileName);
	int Ret = 0;
	while (SizeToRead) {
		int Size = Min(L2_BufferSize, SizeToRead);
		Addr = AT_TensorGetNextPage(Loc, L3_Device, L3_Event, Size, L2_BufferAddr, BaseAddr, ReadSoFar);
		...
		ReadSoFar += Size;
		Item = 0;
		Ret+=AT_FS_SEMIHOST_WRITE(File, Addr, Size);
	}
	AT_FS_SEMIHOST_CLOSE(File);
	AT_FS_SEMIHOST_DEINIT(fs);
	if (Ret != TotalSizeBytes) {printf("Something went wrong with file %s %d bytes\n", FileName, Ret);}
}
